...
set PORT=8082
set SERVER_DIR=/var/www/app-aicode/dist
...
scp -r "%~dp0..\web-aicode-vue\dist\*" root@%SERVER_IP%:%SERVER_DIR%/

echo ===================================================
echo [3/3] 正在上传 Nginx 配置并重载...
echo ===================================================
scp "%~dp0nginx-aicode.conf" root@%SERVER_IP%:/etc/nginx/conf.d/app-aicode.conf
ssh root@%SERVER_IP% "nginx -t && nginx -s reload"

echo ===================================================
echo 恭喜！项目前端静态页面部署成功！
echo 腾讯云前端访问端口: %PORT% (Nginx HTTPS 托管)
echo ===================================================
pause
