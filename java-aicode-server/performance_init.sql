...
CREATE TABLE `perf_task_tran` (
  ...
  -- 1. 生产现状调研情况 (对应模板：生产现状调研情况)  -- 统一使用 tran_ 前缀，一眼识别为生产数据
  ...
  -- 2. 业务交易指标明细要求 (对应模板：业务交易指标明细要求)  -- 统一使用 target_ 前缀，表示性能测试要达到的目标要求
  ...
  -- 3. 交易选择结果 (对应模板：交易选择结果)
  `is_selected` tinyint(1) DEFAULT '1' COMMENT '是否选为性能测试交易*',
  `select_reason` varchar(255) DEFAULT NULL COMMENT '选取原因/备注',
  -- 审计与辅助字段
  `data_source` int(11) DEFAULT '1' COMMENT '数据来源(1:实测, 2:采样折算, 3:经验对标)',
  `is_complex` tinyint(1) DEFAULT '0' COMMENT '是否逻辑复杂/高风险交易',
...
CREATE TABLE `perf_task_batch` (
  ...
  `has_retry` varchar(16) DEFAULT '是' COMMENT '是否有重做机制',
...
-- 性能测试场景定义与实测结果表
CREATE TABLE IF NOT EXISTS `perf_task_scene` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
