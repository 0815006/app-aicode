    ...
    public void processTask(Long taskId) {
        ...
        try {
            ...
            try {
                ...
                log.warn("页面加载超时或失败: {}", task.getUrl());
            }
            ...
            for (String mediaUrl : mediaUrls) {
                try {
                    ...
                    if (fileSize > 0 && fileSize < minSizeBytes) {
                        log.debug("文件太小，跳过: {} ({} bytes)", mediaUrl, fileSize);
                        continue;
                    }
                    ...
                    if (contentType != null && contentType.startsWith("image/")) {
                        ...
                        try (InputStream is = conn.getInputStream();
                            ...
                            log.debug("下载图片: {} ({} bytes)", fileName, totalBytes);
                        }
                        ...
                        try (InputStream is = conn.getInputStream();
                            ...
                            log.debug("下载视频: {} ({} bytes)", fileName, totalBytes);
                        }
                    }
                } catch (Exception e) {
                    log.warn("下载媒体文件失败: {}", mediaUrl, e);
                }
            }

            // 更新任务结果
            String status = (imgCount > 0 || videoCount > 0) ? "SUCCESS" : "FAILED";
