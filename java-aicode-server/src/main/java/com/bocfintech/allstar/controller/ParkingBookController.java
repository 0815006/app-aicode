...
public class ParkingBookController {
     ...
     */
    @GetMapping("/list")
    public ResultBean<List<ParkingBook>> listAll() {
        try {
            List<ParkingBook> list = parkingBookService.getAllRecords();
    ...
    public ResultBean queryMyConfig() {
     ...
     */
    @PostMapping("/delete")
    public ResultBean deleteConfig() {
        String empNo = getEmpNoFromToken();
        if (!StringUtils.hasText(empNo)) {
            return ResultBean.error(ErrorEnum.参数异常, "未登录");
        }

        boolean success = parkingBookService.removeById(empNo);
     ...
     */
    @PostMapping("/check")
    public ResultBean checkParkTasks() {
        String empNo = getEmpNoFromToken();
        if (!StringUtils.hasText(empNo)) {
            return ResultBean.error(ErrorEnum.参数异常, "未登录或Token无效");
        }
     ...
     */
    @PostMapping("/cancel")
    public ResultBean cancelParkTasks() {
        String empNo = getEmpNoFromToken();
        if (!StringUtils.hasText(empNo)) {
            return ResultBean.error(ErrorEnum.参数异常, "未登录或Token无效");
        }
