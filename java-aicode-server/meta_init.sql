CREATE TABLE `meta_file_definition` (
  ...
  `name` varchar(100) NOT NULL COMMENT '文件定义名称',
  `file_name_rule` varchar(200) NOT NULL COMMENT '文件名生成规则 如PAY_yyyyMMdd.txt',
  `encoding` varchar(20) DEFAULT NULL COMMENT '字符集 GBK或UTF-8',
  `line_separator` varchar(10) DEFAULT 'CRLF' COMMENT '换行符 CRLF或LF',
  `parse_mode` varchar(20) DEFAULT 'FIXED' COMMENT '模式 FIXED或DELIMITER',
...
CREATE TABLE `meta_field_definition` (
  ...
  `gen_rule_config` text NOT NULL COMMENT '规则配置JSON 含默认值、填充规则、资源池引用、汇总算法等',
...
CREATE TABLE `meta_resource_pool` (
  ...
  `column_mapping` text DEFAULT NULL COMMENT '列名映射 如0:账号,1:户名',
