...
CREATE TABLE `parking_book` (
  ...
  `email_enabled` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否开启邮件通知（0=关闭，1=开启）',
  `email_user` varchar(50) DEFAULT NULL COMMENT '发送邮件账号用户名（7位工号格式）',
  `email_password` varchar(255) DEFAULT NULL COMMENT '发送邮件账号密码（加密存储）',
  `email_recipient` varchar(255) DEFAULT NULL COMMENT '收件人邮箱地址',
...
CREATE TABLE `performance_resource_info` (
  ...
  `product_id` varchar(32) DEFAULT NULL COMMENT '产品标识与perf_task的batch_no对应',
  `batch_no` varchar(32) NOT NULL COMMENT '性能测试任务-批次 (如：2606)-与perf_task的batch_no对应',
