<!-- src/views/FormatSql.vue -->
<template>
  <div class="format-sql-page">
    <div class="sql-container">
      <!-- 左侧：输入区域 -->
      <div class="input-section card">
        <div class="section-header">
          <h3><i class="el-icon-edit"></i> 输入 SQL</h3>
        </div>
      ...
      <div class="output-section card">
        <div class="section-header">
          <h3><i class="el-icon-s-tools"></i> 格式化结果</h3>
        ...
        <div class="output-content">
          <div class="sql-wrapper">
            <div v-if="formattedSql" class="sql-display">
  ...
  methods: {
    ...
    formatSql() {
      if (!this.inputSql.trim()) {
        this.$message.warning('请输入SQL字符串')
        return
      }

      try {
        this.formattedSql = format(this.inputSql, {
          language: 'mysql',
          keywordCase: 'upper',
        ...
        this.$message.error('SQL格式化失败: ' + (error.message || '请检查SQL语法'))
        this.formattedSql = ''
      }
    },
    copyFormattedSql() {
